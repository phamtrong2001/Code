...
void solveProblem()
{
  int temp = 0, minute = 1;
  FORU(i,2,n)
    if(a[1][i]) temp += 1 << (i-1), mask[1][i] = 1, flower[0]++;
  while(!mark[temp] && minute + 1 <= t)
  {
  ...
  long long Res = flower[0] + flower[minute];
  long long round = (t - minute) / (minute - mark[temp]);
  Res += (flower[minute] - flower[mark[temp]]) * round;
  round = t - minute - round * (minute - mark[temp]);
  Res += flower[mark[temp]+round] - flower[mark[temp]];
  cout << Res;
}
